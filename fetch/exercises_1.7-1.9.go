 ...
 * response body to os.Stdout without requiring a buffer large enough to hold the entire stream. Be sure to checkk the
...
func main() {
	for _, url := range os.Args[1:] {
		...
		var b bytes.Buffer
		byteCount, err := io.Copy(&b, resp.Body)
		fmt.Printf("%d bytes copied from %s\n\n", byteCount, url)
		resp.Body.Close()
		if err != nil {
			fmt.Fprintf(os.Stderr, "fetch: reading %s: %v\n", url, err)
			os.Exit(1)
		}
		fmt.Printf("%s\nStatus: %s\n", b.String(), resp.Status)
	}
}
...
4154 bytes copied from http://gopl.io
